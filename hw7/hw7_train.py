...
class StudentNet(nn.Module):
    ...
    def __init__(self, base=16, width_mult=1):
        ...
        multiplier = [4, 4, 8, 8, 16, 16, 16, 16]
...
optimizer2 = optim.AdamW(teacher_net.parameters(), lr=1e-5)
...
def run_epoch(dataloader, update=True, alpha=0.5):
    ...
    for now_step, batch_data in enumerate(dataloader):
        ...
        if update:
            ...
            # T=20是原始論文的參數設定。
            loss = loss_fn_kd(logits, hard_labels, soft_labels, 20, alpha) # + loss_fn_kd(soft_labels, hard_labels, logits, 20, 0.2)
            loss.backward()
            optimizer.step()
            optimizer2.step()
        else:
...
for epoch in range(200):
    ...
    print('epoch {:>3d}: {:6.4f}, train loss: {:6.4f}, acc {:6.4f} valid loss: {:6.4f}, acc {:6.4f}'.format(
        epoch, time.time()-start_time, train_loss, train_acc, valid_loss, valid_acc))
